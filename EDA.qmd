...
# Fetch the data
gdp_percap <- read.csv("https://ourworldindata.org/grapher/gdp-per-capita-worldbank.csv?v=1&csvType=full&useColumnShortNames=true")
...
# Filter and plot
sdi_data <- cp %>%
  filter(sex == "Both", location_type == "SDI Group", !is.na(Rate)) %>%
  mutate(Rate = Rate / 1000)

# Check if anything is returned
print(unique(sdi_data$location_type))
print(table(sdi_data$location))

# Generate plot
plot_ly(sdi_data, x = ~year, y = ~Rate, color = ~location, type = 'scatter', mode = 'lines') %>%
  layout(
    title = list(text = "Pneumonia Mortality by SDI Group"),
